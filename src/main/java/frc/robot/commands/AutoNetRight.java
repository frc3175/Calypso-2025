...
public class AutoNetRight extends Command {
 ...
 PathConstraints constraints = new PathConstraints(4, 2, 2 * Math.PI, 4 * Math.PI); // The constraints for this path.
  ...
  public AutoNetRight(CommandSwerveDrivetrain drivetrain) {
  ...
  public void initialize() {
    ...
    if (ally.isPresent()) {
      if (ally.get() == Alliance.Red) {
        endPose = Constants.AutoAlignConstants.REDNETRIGHT;
      } else {
        endPose = Constants.AutoAlignConstants.BLUENETRIGHT;
      }
    }
